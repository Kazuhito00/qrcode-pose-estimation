...
def get_args():
    ...
    parser.add_argument("--interval_time", type=int, default=100)
...
def main():
    ...
    while True:
        ...
        cv.putText(
            frame,
            "ESC :Completes Calibration Photographing",
            (10, 55),
            cv.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 0),
        ...
        if ((use_autoappend is True) and found) or (
            (use_autoappend is False and key == 13) and found
        ):  # Enter
            # チェスボードコーナー検出情報を追加
    ...
    if len(image_points) > 0:
        ...
        print("d = " + str(d.ravel()))
        np.savetxt(
            k_filename, K, delimiter=",", fmt="%0.14f"
        )  # 半径方向の歪み係数の保存
        np.savetxt(
            d_filename, d, delimiter=",", fmt="%0.14f"
        )  # 円周方向の歪み係数の保存
        ...
        for i in range(len(object_points)):
            ...
            error = cv.norm(image_points[i], image_points2, cv.NORM_L2) / len(
                image_points2
            )
            mean_error += error
        print(
            "total error: " + str(mean_error / len(object_points))
        )  # 0に近い値が望ましい
    else:
